...
class MsgCommonGroupTest(TestCase):
    ...
    def test_msg_common_group_0118(self):
        ...
        # 验证选中的消息体还在选中状态
        if not (els[2].get_attribute("checked") == "true" and els[1].get_attribute("checked") == "true" and els[
            0].get_attribute("checked") == "true"):
...
class MsgCommonGroupPriorityTest(TestCase):
    ...
    @tags('ALL', 'Priority', 'CMCC', 'high',"CORE")
    ...
    # @unittest.skip('现网环境与原用例描述不符，现网不显示"..."')
    @tags('ALL', 'Priority', 'CMCC', 'high')
    def test_msg_huangmianhua_0049(self):
        ...
        time.sleep(1)
        gcsp.save_group_name()
        gcsp.wait_for_text('群聊设置')
        real_group_name = gcsp.get_element_c('群名称').text
        if not gcsp.is_toast_exist("修改成功"):
            raise AssertionError("群名称更改为新名称失败")
        time.sleep(1)
        gcsp.click_back()
        ...
        try:
            ...
            traceback.print_exc()
            pass
        mess.selecting_one_group_click_by_name(real_group_name)
        # 恢复群名
        gcp.click_setting()
        gcsp.wait_for_page_load()
        gcsp.click_modify_group_name()
    ...
    def test_msg_huangmianhua_0401(self):
        ...
        try:
            ...
            traceback.print_exc()
            pass
        self.msg_huangmianhua_0401(real_group_name)

    def msg_huangmianhua_0401(self, real_group_name):
        mess = MessagePage()
        mess.selecting_one_group_click_by_name(real_group_name)
        # 恢复群名
        gcp = GroupChatPage()
...
class MsgCommonGroupAllTest(TestCase):
    ...
    @tags('ALL', 'CMCC', 'group_chat', 'full', 'high',"CORE")
    ...
    @tags('ALL', 'CMCC', 'group_chat', 'full', 'full-yyx', 'high',"CORE")
    ...
    def setUp_test_msg_xiaoqiu_0288():
        Preconditions.select_mobile('Android-移动')
        Preconditions.make_already_in_message_page()
        Preconditions.build_one_new_group("138138138")
    ...
    @tags('ALL', 'CMCC', 'group_chat', 'yx',"CORE")
